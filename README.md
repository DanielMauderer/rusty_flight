...
CI runs in GitHub Actions (`.github/workflows/ci.yml`) on every push and pull
request. It covers the usual Rust hygiene plus embedded-specific checks:

- **`rustfmt`** — formatting (`cargo fmt --all --check`).
- **`clippy`** — lints with warnings denied, built for the `thumbv7em-none-eabihf`
  target.
- **`build`** — cross-compiles all binaries (dev + release) with `flip-link`, so
  the real link step (`link.x` / `defmt.x`) is exercised.
- **`test (compile-only)`** — `cargo test --no-run`.
- **`docs`** — `cargo doc` with warnings denied.
- **`cargo-deny`** — RUSTSEC advisories, license policy, and dependency bans
  (see `deny.toml`).
- **`firmware size`** — reports per-binary flash/RAM usage to the job summary
  (informational).

> **On-target tests run on hardware, not in CI.** The `defmt-test` suites
> (`src/lib.rs`, `tests/integration.rs`) are flashed and executed on the device
> via the `probe-rs` runner in `.cargo/config.toml`. Stock CI runners have no
> debug probe, so CI only verifies the tests *compile and link* for the target
> (`cargo test --no-run`). Run them for real with a board attached:
>
> ```console
> $ cargo test          # runs unit + integration tests on hardware
